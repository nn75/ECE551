...
rectangle_t canonicalize(rectangle_t r) {
  // WRITE THIS FUNCTION
    r.x=min(r.x,r.x+r.width);
    r.y=min(r.y,r.y+r.height);
    r.width=abs(r.width);
...
rectangle_t intersection(rectangle_t r1, rectangle_t r2) {
  // WRITE THIS FUNCTION
    r1.x=min(r1.x,r1.x+r1.width);
    r1.y=min(r1.y,r1.y+r1.height);
    r1.width=abs(r1.width);
    r1.height=abs(r1.height);
    r2.x=min(r2.x,r2.x+r2.width);
    r2.y=min(r2.y,r2.y+r2.height);
    r2.width=abs(r2.width);
    r2.height=abs(r2.height); 
  
    int w1 = r1.x+r1.width;
    int w2 = r2.x+r2.width;
    int h1 = r1.y+r1.height;
    int h2 = r2.y+r2.height;
    r1.width = min(w1,w2)-max(r1.x,r2.x);
    r1.height = min(h1,h2)-max(r1.y,r2.y);
